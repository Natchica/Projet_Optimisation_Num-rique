...
function Algorithme_De_Newton(f::Function,gradf::Function,hessf::Function,x0,options)
    ...
    while flag == -1
        dk = - hessf(xₖ) \ gradf(xₖ)
        xkplus1 = xₖ + dk
        if norm(gradf(xkplus1)) ≤ max(Tol_rel*norm(g0),Tol_abs)
            flag = 0
        elseif (norm(xkplus1 - xₖ) ≤ epsilon*max(Tol_rel*norm(xₖ),Tol_abs))
            flag = 1
        elseif abs(f(xkplus1) - f(xₖ)) ≤ epsilon*max(Tol_rel*abs(f(xₖ)),Tol_abs)
            flag = 2
